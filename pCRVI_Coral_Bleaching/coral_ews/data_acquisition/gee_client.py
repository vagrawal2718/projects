...
class GEEClient:
    ...
    def extract_timeseries_chunked(
        ...
        for year in range(start_year, end_year + 1):
            ...
            # Include reducer in cache filename (avoid caching conflicts)
            reducer_suffix = f"_{reducer}" if reducer != 'mean' else ""
            cache_filename = f"gee_{band}_{chunk_start_str}_{chunk_end_str}{reducer_suffix}.csv"
            ...
            self.logger.info(
                f"Year {year} ({len(all_dfs)+1}/{num_years}): {chunk_start_str} to {chunk_end_str} ({reducer})"
            )
    ...
    def extract_timeseries(
        ...
        simple_reducers = {
            ...
            'max': ee.Reducer.max(),
            'min': ee.Reducer.min(),
            'percentile_90': ee.Reducer.percentile([90]),
            'percentile_95': ee.Reducer.percentile([95])
        }
        
        # Combined reducers (need special handling)
        combined_reducers = ['mean_plus_1sd', 'mean_plus_2sd', 'median_plus_1sd']
        
        all_valid = list(simple_reducers.keys()) + combined_reducers
        
        if reducer not in all_valid:
            raise ValidationError(
        ...
        # Handle combined reducers (mean + SD)
        if reducer in combined_reducers:
            return self._extract_timeseries_with_sd(
                collection, band, reducer, geometry, scale
            )
        
        # Handle simple reducers
        ee_reducer = simple_reducers[reducer]
        ...
        def extract_value(img):
            ...
            # Handle percentile output names
            if 'percentile' in reducer:
                output_name = f'{band}_p{reducer.split("_")[1]}'
            else:
                output_name = band
            
            return ee.Feature(None, {
                'date': ee.Date(img.get('system:time_start')).format('YYYY-MM-dd'),
                'value': stats.get(output_name, stats.get(band))
            })
    ...
    def _extract_timeseries_with_sd(
        self,
        collection,
        band: str,
        reducer: str,
        ...
        """
        Extract time series using mean/median + standard deviation.
        
        This captures warmer areas within the region rather than just the mean,
        which tends to average out localized thermal hotspots.
        ...
        band : str
            Band name
        reducer : str
            'mean_plus_1sd', 'mean_plus_2sd', or 'median_plus_1sd'
        ...
        -------
        pd.DataFrame
            Time series with combined statistic
        """
        # Determine multiplier and base reducer
        if reducer == 'mean_plus_1sd':
            sd_multiplier = 1.0
            use_median = False
        elif reducer == 'mean_plus_2sd':
            sd_multiplier = 2.0
            use_median = False
        elif reducer == 'median_plus_1sd':
            sd_multiplier = 1.0
            use_median = True
        else:
            sd_multiplier = 1.0
        ...
        # Combined reducer for mean/median and stdDev
        if use_median:
            combined_reducer = ee.Reducer.median().combine(
        ...
        def extract_combined(img):
            """Extract mean/median and stdDev, compute combined value."""
            stats = img.select(band).reduceRegion(
            ...
            # Get the values - keys depend on reducer used
            if use_median:
                central = stats.get(f'{band}_median', stats.get(band))
            else:
                central = stats.get(f'{band}_mean', stats.get(band))
            
            stddev = stats.get(f'{band}_stdDev')
            
            # Compute central + multiplier * stdDev
            # Handle potential None values with server-side conditionals
            combined_value = ee.Algorithms.If(
                ee.Algorithms.IsEqual(central, None),
                None,
                ee.Algorithms.If(
                    ee.Algorithms.IsEqual(stddev, None),
                    central,
                    ee.Number(central).add(ee.Number(stddev).multiply(sd_multiplier))
                )
            )
        ...
        try:
            features = collection.map(extract_combined)
            feature_collection = ee.FeatureCollection(features)
            
            self.logger.info(f"Retrieving {reducer} time series from GEE...")
            data = feature_collection.getInfo()
            ...
            for feature in data['features']:
                ...
                if props['value'] is not None:
                    records.append({
                        'date': props['date'],
                        band: props['value'],
                        f'{band}_central': props.get('central'),
                        f'{band}_stddev': props.get('stddev')
                    })
            
            if not records:
                raise GEEError(
                    "No valid data extracted",
                    operation="extract_timeseries_with_sd",
                    context={"band": band, "reducer": reducer}
                )
            ...
            # Log statistics
            central_col = f'{band}_central'
            stddev_col = f'{band}_stddev'
            if central_col in df.columns and stddev_col in df.columns:
                mean_central = df[central_col].mean()
                mean_stddev = df[stddev_col].mean()
                mean_combined = df[band].mean()
                self.logger.info(
                    f"{reducer} extraction complete:\n"
                    f"  Mean of central values: {mean_central:.3f}\n"
                    f"  Mean of stdDev: {mean_stddev:.3f}\n"
                    f"  Mean of combined ({reducer}): {mean_combined:.3f}"
                )
            
            self.logger.info(f"Extracted {len(df)} records using {reducer}")
            return df
            
        except ee.EEException as e:
            raise GEEError(
                f"Failed to extract {reducer} time series",
                operation="extract_timeseries_with_sd",
                context={"band": band, "reducer": reducer},
                original_exception=e
            )
