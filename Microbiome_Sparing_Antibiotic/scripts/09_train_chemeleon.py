#!/usr/bin/env python3
"""
06_train_dmpnn.py -- Phase 3C: CheMeleon Foundation Model Training (Chemprop v2)
...
# Full metrics suite (always available since it's in our own utils)
try:
...
# CheMeleon saves to its own model directory
# Set CHEMELEON_FREEZE=1 to run frozen-encoder variant (trains only FFN head)
FREEZE_MODE = os.environ.get('CHEMELEON_FREEZE', '0') == '1'
if FREEZE_MODE:
    CHEMELEON_DIR = os.path.join(config.MODELS_DIR, 'chemeleon_frozen')
    METRICS_FILENAME = 'chemeleon_frozen_cv_metrics.json'
    SCREENING_PREFIX = 'chemeleon_frozen'
    PHASE_NAME = "Phase 3C: CheMeleon FROZEN Encoder (FFN-only)"
else:
    CHEMELEON_DIR = os.path.join(config.MODELS_DIR, 'chemeleon')
    METRICS_FILENAME = 'chemeleon_cv_metrics.json'
    SCREENING_PREFIX = 'chemeleon'
    PHASE_NAME = "Phase 3C: CheMeleon Foundation Model Fine-Tuning"

logger = setup_logging('phase3c', log_dir=config.LOGS_DIR)
...
def train_chemprop_model(
    ...
    gpu : bool
        Whether to use GPU.
    freeze_encoder : bool
        If True, freeze MPNN encoder and only train FFN head.
        Uses two-step: load CheMeleon .pt with --checkpoint --freeze-encoder.
        If False (default), fine-tune entire model with low learning rate.
    ...
    if version_type == 'v1':
        # CheMeleon requires chemprop v2.2+. v1 fallback should not be reached.
        logger.error("CheMeleon requires chemprop v2.2+. v1 not supported.")
        return False
    else:
        ...
        if val_csv and os.path.exists(val_csv):
            ...
            # splits JSON: list of dicts with "train", "val", "test" keys
            # (chemprop v2.2 format per docs)
            train_indices = list(range(n_train))
            val_indices = list(range(n_train, n_train + n_val))
            splits_file = os.path.join(save_dir, 'splits.json')
            with open(splits_file, 'w') as sf:
                _json.dump([{"train": train_indices, "val": val_indices, "test": []}], sf)
        ...
        # ================================================================
        # FROZEN ENCODER MODE (two-step approach)
        # ================================================================
        # If freeze_encoder=True, find CheMeleon .pt file (auto-downloaded
        # to ~/.chemprop/) and use --checkpoint --freeze-encoder.
        # This trains ONLY the FFN head (~600 params). The 10M-param
        # pretrained MPNN is a fixed feature extractor.
        # ================================================================
        chemeleon_pt = None
        if freeze_encoder:
            for search_dir in [
                os.path.expanduser('~/.chemprop'),
                os.path.join(config.PROJECT_DIR, '.chemprop'),
            ]:
                if os.path.isdir(search_dir):
                    for f in os.listdir(search_dir):
                        if 'chemeleon' in f.lower() and f.endswith('.pt'):
                            chemeleon_pt = os.path.join(search_dir, f)
                            break
                if chemeleon_pt:
                    break

            if chemeleon_pt is None:
                # Trigger download by doing a quick --from-foundation import
                logger.info("    Downloading CheMeleon for frozen mode...")
                dl_cmd = base_cmd + [
                    'train', '--data-path', data_path,
                    '--task-type', 'classification',
                    '--output-dir', os.path.join(save_dir, '_dl_tmp'),
                    '--from-foundation', 'CheMeleon',
                    '--epochs', '0',
                    '--smiles-columns', 'smiles', '--target-columns', 'label',
                    '--num-workers', '0',
                    '--split', 'RANDOM', '--split-sizes', '1.0', '0.0', '0.0',
                    '--accelerator', 'cpu',
                ]
                subprocess.run(dl_cmd, capture_output=True, timeout=300)
                for search_dir in [os.path.expanduser('~/.chemprop'),
                                   os.path.join(config.PROJECT_DIR, '.chemprop')]:
                    if os.path.isdir(search_dir):
                        for f in os.listdir(search_dir):
                            if 'chemeleon' in f.lower() and f.endswith('.pt'):
            ...
            if chemeleon_pt is None:
                logger.warning("    CheMeleon .pt not found. Falling back to fine-tune mode.")
                freeze_encoder = False
            else:
                logger.info(f"    FROZEN ENCODER: {chemeleon_pt}")

        if freeze_encoder and chemeleon_pt:
            cmd = base_cmd + [
                'train',
                '--data-path', data_path,
                '--task-type', 'classification',
                '--output-dir', save_dir,
                '--checkpoint', chemeleon_pt,
                '--freeze-encoder',
                # Frozen MPNN = only FFN trains (~600 params).
                # Can use more epochs + higher LR (no overfitting risk in encoder).
                '--epochs', '10',
                '--batch-size', '32',
                '--init-lr', '1e-4',
                '--max-lr', '1e-3',
            ...
            '--from-foundation', 'CheMeleon',
            # ================================================================
            # FOUNDATION MODEL FINE-TUNING SETTINGS
            # ================================================================
            # CheMeleon: 6-layer 2048-dim D-MPNN pretrained on 1M molecules.
            #
            # --from-foundation loads pretrained MPNN and creates a NEW FFN.
            # Both are trainable (--freeze-encoder requires --checkpoint, not
            # --from-foundation; they are separate code paths in chemprop v2).
            #
            # Three critical differences from training D-MPNN from scratch:
            #
            # 1. LOW LEARNING RATE: default max-lr=1e-3 destroys pretrained
            #    weights. Use 1e-4 (standard for foundation model fine-tuning).
            # 2. FEW EPOCHS: pretrained representations converge in 3-5 epochs.
            #    50 epochs caused train_loss=0.05, val_loss=1.58 (catastrophic).
            # 3. HIGH DROPOUT: 10M params on 2-5K compounds needs regularization.
            #
            # FFN dims are NOT overridden. The --from-foundation path creates
            # a new FFN using chemprop defaults (300-dim, 1-layer), which is
            # appropriate since the 2048-dim MPNN output is the rich feature.
            # ================================================================
            '--epochs', '5',
            '--batch-size', '32',
            '--init-lr', '1e-5',
...
def train_dmpnn_with_cv(
    ...
    full_metrics_agg = {}
    if HAS_FULL_METRICS:
        fold_full_list = [m.get('full_metrics', {}) for m in fold_metrics
                          if m.get('full_metrics')]
        if fold_full_list:
            full_metrics_agg = aggregate_fold_metrics(fold_full_list)
...
def screen_hub_dmpnn(
    ...
    hub_smiles_csv = os.path.join(config.DATA_DIR, 'chemeleon_input', 'hub_screen.csv')

    pathogen_probs = {}
    for key, model_dir in pathogen_model_dirs.items():
        preds_csv = os.path.join(config.SCREENING_DIR, f'{SCREENING_PREFIX}_preds_{key}.csv')
    ...
    for pathogen_key, p_path in pathogen_probs.items():
        for gut_key, p_gut in gut_probs.items():
            ...
            csv_path = os.path.join(config.SCREENING_DIR, f'{SCREENING_PREFIX}_ranked_{combo_key}.csv')
...
def run_unit_tests() -> bool:
    ...
    print("Running Phase 3C (CheMeleon) unit tests...")
    n_pass = 0
    n_fail = 0
...
def main():
    ...
    start_time = log_phase_start(logger, PHASE_NAME)

    ckpt_path = os.path.join(config.CHECKPOINTS_DIR, 'phase3c_master.json')
    ...
    os.makedirs(CHEMELEON_DIR, exist_ok=True)
    ...
    try:
        ...
        if dm.restore_dmpnn_models(config.PROJECT_DIR):
            ...
            pt_count = 0
            for root, dirs, files in os.walk(CHEMELEON_DIR):
                pt_count += len([f for f in files if f.endswith(('.pt', '.ckpt'))])
            metrics_path = os.path.join(config.RESULTS_DIR, METRICS_FILENAME)
            n_screen = len([f for f in os.listdir(config.SCREENING_DIR) if f.startswith(f'{SCREENING_PREFIX}_ranked_')]) if os.path.isdir(config.SCREENING_DIR) else 0
            if pt_count >= 5 and os.path.exists(metrics_path) and n_screen >= 12:
                ...
                save_checkpoint({'status': 'complete'}, os.path.join(config.CHECKPOINTS_DIR, 'phase3c_master.json'), logger)
    ...
    gut_final_dirs = {}
    quality_report = {}
    dmpnn_input_dir = os.path.join(config.DATA_DIR, 'chemeleon_input')
    ...
    model_num = 0
    logger.info(f"\n{'='*60}")
    logger.info(f" PATHOGEN CheMeleon MODELS ({len(config.PATHOGENS)} of {total_dmpnn_models} total)")
    logger.info(f"{'='*60}")
    ...
    for key in config.PATHOGENS:
        ...
        model_dir = os.path.join(CHEMELEON_DIR, key)
        try:
            result = train_dmpnn_with_cv(
                data_csv, folds, f'chemeleon_{key}', model_dir, version_type, gpu_available
            )
        except Exception as e:
        ...
        oof_path = os.path.join(CHEMELEON_DIR, key, f'chemeleon_{key}_oof.csv')
        oof_df = pd.DataFrame({
    ...
    # ==================================================================
    logger.info(f"\n{'='*60}")
    logger.info(f" GUT HARM CheMeleon MODELS (3 thresholds)")
    logger.info(f"{'='*60}")
    ...
    if os.path.exists(maier_folds_path):
        ...
        for t in config.HARM_THRESHOLDS:
            ...
            model_dir = os.path.join(CHEMELEON_DIR, name)
            result = train_dmpnn_with_cv(
                data_csv, maier_folds, f'chemeleon_{name}', model_dir, version_type, gpu_available
            )
            ...
            oof_df.to_csv(os.path.join(model_dir, f'chemeleon_{name}_oof.csv'), index=False)
    ...
    # ==================================================================
    cv_metrics_path = os.path.join(config.RESULTS_DIR, METRICS_FILENAME)
    cv_export = {}
    for name, result in all_cv_results.items():
        cv_export[name] = {
            ...
            'fold_metrics': result['fold_metrics'],
            'full_metrics_agg': result.get('full_metrics_agg', {}),
        }
