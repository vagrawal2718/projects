...
def train_dmpnn_with_cv(
    ...
    for fold_idx in range(n_folds):
        ...
        # --- FOLD SKIP: if predictions already exist, reuse them ---
        ...
        if os.path.exists(preds_csv):
            try:
                ...
                probs = df_preds[pred_col[0]].values.astype(float)
                if len(probs) == len(test_idx):
                    ...
                    logger.info(f"    Fold {fold_idx}: SKIP (cached preds), "
                                f"ROC-AUC={roc_auc:.4f}, PR-AUC={pr_auc:.4f}")
                    continue
            except Exception as e:
                logger.warning(f"    Fold {fold_idx}: cached preds unreadable ({e}), retraining")
