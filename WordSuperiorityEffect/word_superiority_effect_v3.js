...
let expName = 'word_superiority_effect_v3';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'conditions3.xlsx', 'path': 'conditions3.xlsx'},
    {'name': 'bg.jpg', 'path': 'bg.jpg'},
  ]
});
    ...
    if (start_mouse.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          ...
          gotValidClick = false;
          start_mouse.clickableObjects = eval(start)
    ...
    if (mouse.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          // check if the mouse was inside our 'clickable' objects
          gotValidClick = false;
          mouse.clickableObjects = eval([lefttxt, righttxt])
          ;// make sure the mouse's clickable objects are an array
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    current_rt = null;
    if (key_resp.keys) {
        current_corr = key_resp.corr;
        current_rt = key_resp.rt;
    } else {
        if (mouse.clicked_name) {
            if (mouse.clicked_name) {
                clicked_button = mouse.clicked_name[0];
                if (((clicked_button === "lefttxt") && (corr_ans === "left"))) {
                    current_corr = 1;
                } else {
                    if (((clicked_button === "righttxt") && (corr_ans === "right"))) {
                        current_corr = 1;
                    } else {
                        current_corr = 0;
                    }
                ...
                if (mouse.time) {
                    current_rt = mouse.time.slice((- 1))[0];
                }
            } else {
                current_corr = 0;
                if (mouse.time) {
    ...
    if ((condition === "word")) {
        word_accuracies.push(current_corr);
        if ((current_rt !== null)) {
            word_rts.push(current_rt);
        }
    } else {
        if ((condition === "nonword")) {
            nonword_accuracies.push(current_corr);
            if ((current_rt !== null)) {
                nonword_rts.push(current_rt);
            }
        }
    }
    psychoJS.experiment.addData("correct", current_corr);
    psychoJS.experiment.addData("rt", current_rt);
