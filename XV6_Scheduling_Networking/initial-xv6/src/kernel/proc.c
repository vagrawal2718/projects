...
{
  if (p->priority < 3 && p->queue_time >= 48)
  {
    p->priority++;
    p->time_slice = 1 << p->priority;
    p->queue_time = 0;
  }
}*/
...
/*void boost_priority(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state != UNUSED)
    {
      // Reset all runnable processes to the highest priority queue (Queue 0)
      p->priority = 0;
      p->queue_time = 0;
      p->time_slice = 1;  // Time slice for the highest priority queue
    }
    release(&p->lock);
  }
}*/
...
found:
  ...
  /* Initialize the syscall_counts array to zero.
  for (int i = 0; i < 32; i++) {
    p->syscall_counts[i] = 0;   // Initialize syscall counts to zero
  }*/
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Insert the syscall count copying code here
  /*for(int i = 0; i < 32; i++){
    //p->syscall_counts[i] = np->syscall_counts[i];
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    if (p)
    {
      //printf("Picked process PID: %d, Tickets: %d\n", p->pid, p->tickets);
      ...
      if (p->state == RUNNABLE)
      {
        ...
        //printf("Process PID: %d, Total runtime: %d ticks\n", p->pid, p->rtime);
        c->proc = 0;
      }
      release(&p->lock);
    }
    ...
    {
       //printf("No runnable process was picked.\n");
    }
#endif
    ...
    for (int priority = 0; priority < 4; priority++)
    {
      for (p = proc; p < &proc[NPROC]; p++)
      {
        ...
        {
          //log_queue_transition(p, priority, ticks); 
          //printf("PID: %d, Queue: %d, Tick: %d\n", p->pid, priority, ticks);
          p->state = RUNNING;
          c->proc = p;
          swtch(&c->context, &p->context);
          c->proc = 0;
